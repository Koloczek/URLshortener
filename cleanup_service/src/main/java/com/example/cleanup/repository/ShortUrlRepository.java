...
public interface ShortUrlRepository extends CassandraRepository<ShortUrlEntity, String> {
    ...
    List<ShortUrlEntity> findByCreationTimeBefore(long timestamp);

    @Query("SELECT * FROM short_url_entity WHERE last_access_time < ?0 ALLOW FILTERING")
    List<ShortUrlEntity> findByLastAccessTimeBefore(long timestamp);

    @Query("SELECT * FROM short_url_entity WHERE expiration_time < ?0 ALLOW FILTERING")
    List<ShortUrlEntity> findByExpirationTimeBefore(long timestamp);
}
